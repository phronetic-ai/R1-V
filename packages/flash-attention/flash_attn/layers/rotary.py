...
def apply_rotary_emb_torch(x, cos, sin, interleaved=False):
    # Ensure x and cosine embeddings have the same dtype
    x, cos, sin = x.to(torch.bfloat16), cos.to(torch.bfloat16), sin.to(torch.bfloat16)
    assert x.dtype == cos.dtype, "Input tensor x and cosine embeddings must have the same dtype"
    roDim = cos.shape[-1] * 2
    assert roDim <= x.shape[-1], "Rotary dimension exceeds input dimension"
    repeatPattern = "... d -> ... 1 (2 d)" if not interleaved else "... d -> ... 1 (d 2)"
    cosRepeated = repeat(cos, repeatPattern)
    sinRepeated = repeat(sin, repeatPattern)
    rotaryPart = x[..., :roDim]
    rotatedPart = rotate_half(rotaryPart, interleaved)
    applied = rotaryPart * cosRepeated + rotatedPart * sinRepeated

    return torch.cat([applied, x[..., roDim:]], dim=-1)
